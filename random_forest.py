...
def RandomForest(X, y, profundidade, tam_teste, draw_graph):
    ...
    if draw_graph:
        ...
        # Plot da importância das características no segundo subplot
        importances = model.feature_importances_
        feature_names = X.columns
        indices = np.argsort(importances)[::-1]  # Ordenar por importância em ordem decrescente
        ax2.bar(range(len(feature_names)), importances[indices], align='center')
